...
public final class ServiceManager {
    ...
    private static final String TAG = "plugin_serviceManager";
    ...
    public void onStartCommand(Intent proxyIntent, int flags, int startId){
        Intent targetIntent = proxyIntent.getParcelableExtra(AMSHookHelper.EXTRA_TARGET_INTENT);
        Log.d(TAG, "onStartCommand() called with " + "targetIntent = [" + targetIntent + "], startId = [" + startId + "]");
        ServiceInfo serviceInfo = selectPluginService(targetIntent);
    ...
    public int stopService(Intent targetIntent) {
        ...
        if (mServiceMap.isEmpty()) {
            ...
            Context appContext = PluginManagerImpl.getInstance().getContext();
    ...
    private ServiceInfo selectPluginService(Intent pluginIntent) {
        for (ComponentName componentName : mServiceInfoMap.keySet()) {
            Log.i(TAG,"componentName ："+componentName.toString());
        }
        for (ComponentName componentName : mServiceInfoMap.keySet()) {
    ...
    public String selectPluginService(String serviceName){
        for (ComponentName componentName : mServiceInfoMap.keySet()) {
            Log.i(TAG,"componentName ："+componentName.toString());
            if (TextUtils.equals(componentName.getClassName(),serviceName)){
    ...
    private void proxyCreateService(ServiceInfo serviceInfo) throws Exception {
        ...
        serviceInfo.applicationInfo.packageName = PluginManagerImpl.getInstance().getContext().getPackageName();
     ...
     */
    public void preLoadServices(File apkFile) throws ClassNotFoundException, NoSuchMethodException, IllegalAccessException, InstantiationException, InvocationTargetException, NoSuchFieldException {
        Class<?> packageParserClass = Class.forName("android.content.pm.PackageParser");
        Method parsePackageMethod = packageParserClass.getDeclaredMethod("parsePackage", File.class, int.class);
//        Method parsePackageMethod2 = packageParserClass.getDeclaredMethod("parsePackage", File.class,String.class, DisplayMetrics.class,int.class);
        DisplayMetrics metrics = new DisplayMetrics();
        Log.i("plugin_loader","0");
        Object packageParser = packageParserClass.newInstance();
        Log.i("plugin_loader","1");
        // 首先调用parsePackage获取到apk对象对应的Package对象
        Object packageObj = parsePackageMethod.invoke(packageParser, apkFile, PackageManager.GET_SERVICES);
        Log.i("plugin_loader","2");
